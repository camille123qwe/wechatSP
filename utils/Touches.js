...
class Touches {
  ...
  deleteItem(e, dataList) {  // 删除功能
    console.log(this._getIndex(e));
    let str = dataList.splice(this._getIndex(e), 1)
    let i = e.currentTarget.dataset.mainindex;
    let id = (str[i].cartgoodsitemsid);
    return {
      list: dataList,
      id: id
    }
  }
  deleteCollect(e, dataList) {  // 删除功能
    let str = dataList.splice(this._getIndex(e), 1);
    let id = str[0].goodsInfo.goods36id
    console.log(str[0]);
    return {
      list: dataList,
      id: id
